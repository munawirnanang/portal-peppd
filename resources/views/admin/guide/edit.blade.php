        ...
        <div class="container-fluid" style="width: 100%; margin-right: 0px;">
            ...
            <div class="row">
                        ...
                        <div class="col-md-6">
                            <div class="card sm-12 guide-list-content-card">
                                ...
                                <div class="card-body">
                                    ...
                                    <a href="{{ url('guidelines/'.$list->id.'/edit') }}" class="btn btn-warning">Edit</a>
                ...
                <div class="col-md-4">
                    ...
                    <!-- Form Edit Guide -->
                    <div id="form-edit-guide">
                        <div class="card card-warning">
                            <div class="card-header">
                                <h3 class="card-title"><b>Edit Guide</b></h3>
                            ...
                            <!-- /.card-header -->
                            <!-- form start -->
                            <form action="{{ url('guidelines/'.$guide->id) }}" method="post" enctype="multipart/form-data">
                            @method('patch')
                            @csrf
                            <div class="card-body">
                                <div class="form-group">
                                    ...
                                    <select class="form-control @error('id_category') is-invalid @enderror listCategory" id="edit_category" name="id_category" required>
                                    </select>
                                    @error('id_category')
                                ...
                                <div class="form-group">
                                    ...
                                    <input type="text" class="form-control @error('name') is-invalid @enderror" id="edit_name" name="name" placeholder="Enter Name" value="{{ $guide->name }}" required>
                                    @error('name')
                                        <div class="invalid-feedback">
                                            {{ $message }}
                                        </div>
                                ...
                                <div class="form-group">
                                    ...
                                    <textarea class="form-control" id="edit_description" name="description" rows="2">{{ $guide->description }}</textarea>
                                </div>
                                <div class="row">
                                    <div class="col-md-6">
                                        <div class="form-group">
                                            <label for="edit_title_picture">Cover Picture</label>
                                            <div class="input-group">
                                                <div class="custom-file">
                                                    <input type="file" id="edit_title_picture" name="title_picture" accept="image/*" value="{{ $guide->title_picture }}">
                                                    <label class="custom-file-label" for="edit_title_picture">Choose file</label>
                                                </div>
                                            </div>
                                        </div>
                                    </div>
                                    <div class="col-md-6">
                                        @if($guide->title_picture == NULL)
                                            <img id="preview_edit_title_picture" alt="">
                                        @else
                                            <img id="preview_edit_title_picture" src="{{ asset('guide/'.Str::slug($guide->name, '-').'/'.$guide->title_picture) }}" alt="{{ $guide->title_picture }}" class="img-thumbnail" style="max-height: 200px; margin-top: 10px;">
                                        @endif
                                    </div>
                                </div>
                                <div class="form-group">
                                    ...
                                    <div class="input-group">
                                        <div class="custom-file">
                                            <input type="file" class="custom-file-input" id="edit_file" name="file" value="{{ $guide->file }}">
...
<script>
    $(document).ready(function(){
        ...
        $(window).scroll(function() {
            ...
            if (scrolled > 400) {
                $("#form-edit-guide").addClass("position-fixed");
                $("#form-edit-guide").css({'top' : '0', 'width' : (32 * widthcontainer) / 100, 'margin-top' : '1%'});
            }else{
                $("#form-edit-guide").removeClass("position-fixed");
                $("#form-edit-guide").css({'top' : '', 'width' : '', 'margin-top' : ''});
            }
        });
...
<script type="text/javascript">   
    $(document).ready(function (e) {
        $('#edit_title_picture').change(function(){

            let reader = new FileReader();
            reader.onload = (e) => { 
                $('#preview_edit_title_picture').attr('src', e.target.result);
            }
            reader.readAsDataURL(this.files[0]); 

            $("#preview_edit_title_picture").css({"max-height" : "200px", "margin-top" : "10px"});
            $("#preview_edit_title_picture").addClass("img-thumbnail");
            $("#preview_edit_title_picture").attr("alt", this.value.replace(/C:\\fakepath\\/i, ''));
